...
registerBlockType<{ label: string, name: string, required: boolean, }>( 'form-field-blocks/textarea', {
	attributes: {
		label: {
			type: 'string',
			source: 'html',
	...
	edit: ( { className, setAttributes, attributes: { label, name, required } } ) => {
		return (
		...
		return (
			<div className="wp-block-form-field-blocks-form__row">
				<label>
					...
					{ required ? <textarea name={ name } required /> : <textarea name={ name } /> }
				</label>
			</div>
		);
	}
